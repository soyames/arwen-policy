...
class UNESCOAdapter(GenericSourceAdapter):
    """UNESCO-specific adapter refinement"""

    def __init__(self, source: SourceDefinition):
        super().__init__(source)
        self.specific_domains = ["unesco.org", "www.unesco.org"]
        self.domain_patterns = ["culture", "education", "science", "communication", "heritage"]

    def _discover_docs(self, domain: str) -> List[str]:
        """Discover documents specific to UNESCO with pattern matching"""
        root = _root_url(self.source)
        candidates = [root]

        # UNESCO specific discovery paths
        domain_specific_paths = [
            f"{root}education",
            f"{root}culture",
            f"{root}science",
            f"{root}communication",
            f"{root}heritage",
            f"{root}reports",
            f"{root}publications",
            f"{root}policy-papers"
        ]

        # Add convention and program specific paths
        candidates.extend([
            f"{root}conventions",
            f"{root}world-heritage",
            f"{root}intellectual-property",
            f"{root}open-science",
            f"{root}ai-ethics",
            f"{root}media-development",
            f"{root}literacy"
        ])

        current_year = datetime.now().year
        candidates.extend([
            f"{root}{current_year}/global-education-monitoring",
            f"{root}{current_year}/science-report",
            f"{root}{current_year}/culture-report"
        ])

        return [url for url in candidates if url.startswith(f"https://{domain}")]

    def discover(self) -> "SourceDiscoveryBundle":
        """Enhanced discovery for UNESCO sources"""
        ...
        # Domain-specific discovery paths for UNESCO
        domain_specific_paths = self._discover_docs(self.source.domain)

        # Add specialized UNESCO programs and conventions
        candidates = list(set(
