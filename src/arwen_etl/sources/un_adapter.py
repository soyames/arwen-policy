...
class UNAdapter(GenericSourceAdapter):
    """United Nations (UN) specific adapter refinement"""

    def __init__(self, source: SourceDefinition):
        super().__init__(source)
        self.specific_domains = ["un.org", "www.un.org", "undocs.org"]
        self.domain_patterns = ["resolution", "document", "report", "agenda", "convention"]

    def _discover_docs(self, domain: str) -> List[str]:
        """Discover documents specific to UN with pattern matching"""
        root = _root_url(self.source)
        candidates = [root]

        # UN specific discovery paths
        domain_specific_paths = [
            ...
            f"{root}papers",
            f"{root}publications",
            f"{root}press-releases"
        ]

        # Add current year section and topical areas
        current_year = datetime.now().year
        candidates.extend([
            f"{root}{current_year}",
            f"{root}{current_year}/sustainable-development",
            f"{root}{current_year}/climate-change",
            f"{root}{current_year}/human-rights",
            f"{root}{current_year}/disarmament",
            f"{root}{current_year}/development"
        ])

        return [url for url in candidates if url.startswith(f"https://{domain}")]

    def discover(self) -> "SourceDiscoveryBundle":
        """Enhanced discovery for UN sources"""
        ...
        # Domain-specific discovery paths for UN
        domain_specific_paths = self._discover_docs(self.source.domain)

        # Add current affairs and thematic document sections
        candidates = list(set(
            self._candidate_urls(self.source) +
            domain_specific_paths +
            [
                f"{root}current-events",
                f"{root}events",
                f"{root}meetings",
                f"{root}conferences",
                f"{root}consultations",
                f"{root}publications"
            ]
        ))
