...
class OECDAdapter(GenericSourceAdapter):
    """OECD-specific adapter refinement"""

    def __init__(self, source: SourceDefinition):
        super().__init__(source)
        self.specific_domains = ["oecd.org", "www.oecd.org"]
        self.domain_patterns = ["publication", "report", "analysis", "statistics", "country-study"]

    def _discover_docs(self, domain: str) -> List[str]:
        """Discover documents specific to OECD with pattern matching"""
        root = _root_url(self.source)
        candidates = [root]

        # OECD specific discovery paths
        domain_specific_paths = [
            f"{root}publications",
            f"{root}reports",
            f"{root}analysis",
            f"{root}statistics",
            f"{root}country-studies",
            f"{root}data",
            f"{root}working-papers"
        ]

        # Add sector-specific pathways
        candidates.extend([
            f"{root}energy",
            f"{root}environment",
            f"{root}economic-policy",
            f"{root}digital",
            f"{root}governance"
        ])

        # Add annual reports and country profiles
        current_year = datetime.now().year
        candidates.extend([
            f"{root}{current_year}/annual-report",
            f"{root}{current_year}/country-profiles",
            f"{root}{current_year}/statistical-database"
        ])

        return [url for url in candidates if url.startswith(f"https://{domain}")]

    def discover(self) -> "SourceDiscoveryBundle":
        """Enhanced discovery for OECD sources"""
        ...
        # Domain-specific discovery paths for OECD
        domain_specific_paths = self._discover_docs(self.source.domain)

        # Add OECD standard report categories
        std_report_paths = [
            f"{root}statistical-database/categories",
            f"{root}standards/official-statistics",
            f"{root}publications-by-category"
        ]
        ...
        candidates = list(set(
            ...
            [
                ...
                f"{root}policies/",
                f"{root}openness/",
                f"{root}workshop-reports/"
            ]
        ))

        candidates = candidates[:90]  # Handle comprehensive OECD content
