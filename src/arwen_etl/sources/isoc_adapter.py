...
class ISOCAdapter(GenericSourceAdapter):
    """Internet Society (ISOC) adapter refinement"""

    def __init__(self, source: SourceDefinition):
        super().__init__(source)
        self.specific_domains = ["isoc.org", "www.isoc.org"]
        self.domain_patterns = ["standard", "foundation", "community", "event", "conference"]

    def _discover_docs(self, domain: str) -> List[str]:
        """Discover documents specific to ISOC with pattern matching"""
        root = _root_url(self.source)
        candidates = [root]

        # ISOC specific discovery paths
        ...
        # Add community and program pathways
        candidates.extend([
            f"{root}global-internet-forum",
            f"{root}internet-society-forum",
            f"{root}digital-development",
            f"{root}internet-governance",
            f"{root}youth-ethanol",
            f"{root}women-in-tech"
        ])

        current_year = datetime.now().year
        candidates.extend([
            f"{root}{current_year}/annual-report",
            f"{root}{current_year}/member-survey",
            f"{root}{current_year}/innovation Awards"
        ])

        return [url for url in candidates if url.startswith(f"https://{domain}")]

    def discover(self) -> "SourceDiscoveryBundle":
        """Enhanced discovery for ISOC sources"""
        ...
        # Domain-specific discovery paths for ISOC
        domain_specific_paths = self._discover_docs(self.source.domain)

        # Add governance and standards pathways
        candidates = list(set(
