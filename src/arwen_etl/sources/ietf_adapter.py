...
class IETFAdapter(GenericSourceAdapter):
    """IETF-specific adapter refinement"""

    def __init__(self, source: SourceDefinition):
        super().__init__(source)
        self.specific_domains = ["ietf.org", "www.ietf.org", "datatracker.ietf.org"]
        self.domain_patterns = ["draft", "rfc", "std", "agenda", "minutes"]

    def _discover_docs(self, domain: str) -> List[str]:
        """Discover documents specific to IETF with pattern matching"""
        root = _root_url(self.source)
        candidates = [root]

        # IETF specific discovery paths
        domain_specific_paths = [
            ...
            f"{root}mail",
            f"{root}groups",
            f"{root}working-groups",
            f"{root}tools",
            f"{root}mailman"
        ]

        # Add RFC and draft specific paths
        candidates.extend([
            f"{root}rfc/rfc-index.xml",
            f"{root}ids/",
            f"{root}draft-ietf-",
            f"{root}ietf/",
            f"{root}openmeetings/"
        ...
        # Add current working group meetings and interim
        current_year = datetime.now().year
        candidates.extend([
            f"{root}{current_year}-",
            f"{root}{current_year}/",
            f"{root}interim/{current_year}-",
            f"{root}sessions/{current_year}/"
        ])

        return [url for url in candidates if any(domain in url for domain in self.specific_domains)]

    def discover(self) -> "SourceDiscoveryBundle":
        """Enhanced discovery for IETF sources"""
        ...
        # Domain-specific discovery paths for IETF
        domain_specific_paths = self._discover_docs(self.source.domain)

        # Add sitemap and feed discovery plus standards-specific paths
        candidates = list(set(
            ...
            [
                ...
                f"{root}rss/",
                f"{root}blog/",
                f"{root}datatracker/api/",
                f"{root}datatracker/search/"
            ]
        ))
