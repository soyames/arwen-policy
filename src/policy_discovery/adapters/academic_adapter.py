...
class AcademicAdapter(BaseAdapter):
    DEFAULT_SEEDS = [
        "https://repository.worldpolicyacademy.org/",
        "https://www.poli-sci mejournals.org",
        "https://www.unep-wpp.org/policymaking"
    ]

    def __init__(self, source_id: str = "academic", config: dict = None):
        super().__init__(source_id, config)
        self.seeds = self.config.get("seeds", self.DEFAULT_SEEDS)
        self.max_pages = self.config.get("max_pages", 40)

    def discover_urls(self) -> List[DiscoveredURL]:
        """
        Discover academic policy sources from journals, repositories, and UN documents.
        """
        ...
        for seed in self.seeds[:self.max_pages]:
            try:
                ...
                print(f"Academic adapter error for {seed}: {e}")
        # Deduplicate
        seen = set()
    ...
    def capture(self, url: str) -> List:
        """
        Capture academic policy content and format for ETL.
        """
        try:
            resp = httpx.get(url, timeout=30.0)
            resp.raise_for_status()
            # Parse academic content (placeholder - integrate with actual parser)
            segments = [
                {
                    "text": "Placeholder: Academic policy analysis from {url}",
                    "extraction_method": "academic-html-parsing",
                    "source_id": self.source_id,
            ...
            print(f"Academic capture failed for {url}: {e}")
            return []

    def provenance(self, url: Optional[str] = None) -> Dict[str, Any]:
        """
        Return academic source provenance metadata.
        """
        base = {
            "source": "Academic Institutions/UN Bodies",
            "type": "academic_or_un",
